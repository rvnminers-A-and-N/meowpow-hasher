...
///    restricted by the Ethashprime specification.
...
{
constexpr auto revision = ETHASHPRIME_REVISION;

static constexpr int epoch_length = ETHASHPRIME_EPOCH_LENGTH;
static constexpr int light_cache_item_size = ETHASHPRIME_LIGHT_CACHE_ITEM_SIZE;
static constexpr int full_dataset_item_size = ETHASHPRIME_FULL_DATASET_ITEM_SIZE;
static constexpr int num_dataset_accesses = ETHASHPRIME_NUM_DATASET_ACCESSES;

using epoch_context = ethashprime_epoch_context;
using epoch_context_full = ethashprime_epoch_context_full;

using result = ethashprime_result;
...
/// Alias for ethashprime_calculate_light_cache_num_items().
static constexpr auto calculate_light_cache_num_items = ethashprime_calculate_light_cache_num_items;

/// Alias for ethashprime_calculate_full_dataset_num_items().
static constexpr auto calculate_full_dataset_num_items = ethashprime_calculate_full_dataset_num_items;

/// Alias for ethashprime_calculate_epoch_seed().
static constexpr auto calculate_epoch_seed = ethashprime_calculate_epoch_seed;
...
using epoch_context_ptr = std::unique_ptr<epoch_context, decltype(&ethashprime_destroy_epoch_context)>;

using epoch_context_full_ptr =
    std::unique_ptr<epoch_context_full, decltype(&ethashprime_destroy_epoch_context_full)>;

/// Creates Ethashprime epoch context.
///
/// This is a wrapper for ethashprime_create_epoch_number C function that returns
...
{
    return {ethashprime_create_epoch_context(epoch_number), ethashprime_destroy_epoch_context};
}

inline epoch_context_full_ptr create_epoch_context_full(int epoch_number) noexcept
{
    return {ethashprime_create_epoch_context_full(epoch_number), ethashprime_destroy_epoch_context_full};
}
...
{
    return ethashprime_hash(&context, &header_hash, nonce);
}
...
{
    return ethashprime_verify_final_hash(&header_hash, &mix_hash, nonce, &boundary);
}
...
{
    return ethashprime_verify(&context, &header_hash, &mix_hash, nonce, &boundary);
}
...
///
/// @param seed  Ethashprime seed hash.
...
{
    return *ethashprime_get_global_epoch_context(epoch_number);
}

/// Get global shared epoch context with full dataset initialized.
inline const epoch_context_full& get_global_epoch_context_full(int epoch_number) noexcept
{
    return *ethashprime_get_global_epoch_context_full(epoch_number);
}
}  // namespace ethashprime
