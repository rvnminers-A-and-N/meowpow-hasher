...
constexpr static unsigned int full_dataset_init_size = (1U << 30)*5;
...
static_assert(sizeof(hash1024) == ETHASHPRIME_FULL_DATASET_ITEM_SIZE, "");
static_assert(light_cache_item_size == ETHASHPRIME_LIGHT_CACHE_ITEM_SIZE, "");
static_assert(full_dataset_item_size == ETHASHPRIME_FULL_DATASET_ITEM_SIZE, "");
    ...
    build_light_cache_fn build_fn, int epoch_number, bool full) noexcept
{
    ...
    for (uint32_t i = 0; i < progpowprime::l1_cache_size / sizeof(full_dataset_2048[0]); ++i)
...
using namespace ethashprime;

extern "C" {

ethashprime_hash256 ethashprime_calculate_epoch_seed(int epoch_number) noexcept
{
    ethashprime_hash256 epoch_seed = {};
    for (int i = 0; i < epoch_number; ++i)
        epoch_seed = ethashprime_keccak256_32(epoch_seed.bytes);
    return epoch_seed;
}
...
int ethashprime_calculate_light_cache_num_items(int epoch_number) noexcept
{
    ...
    int num_items = ethashprime_find_largest_prime(num_items_upper_bound);
    return num_items;
}
...
epoch_context* ethashprime_create_epoch_context(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, false);
}

epoch_context_full* ethashprime_create_epoch_context_full(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, true);
}

void ethashprime_destroy_epoch_context_full(epoch_context_full* context) noexcept
{
    ethashprime_destroy_epoch_context(context);
}

void ethashprime_destroy_epoch_context(epoch_context* context) noexcept
{
    context->~epoch_context();
    std::free(context);
}

ethashprime_result ethashprime_hash(
...
bool ethashprime_verify_final_hash(const hash256* header_hash, const hash256* mix_hash, uint64_t nonce,
...
bool ethashprime_verify(const epoch_context* context, const hash256* header_hash,
