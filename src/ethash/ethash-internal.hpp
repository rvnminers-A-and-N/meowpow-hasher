...
/// @file
/// Contains declarations of internal ethashprime functions to allow them to be
/// unit-tested.
...
extern "C" struct ethashprime_epoch_context_full : ethashprime_epoch_context
{
    ethashprime_hash1024* full_dataset;

    constexpr ethashprime_epoch_context_full(int epoch, int light_num_items,
        const ethashprime_hash512* light, const uint32_t* l1, int dataset_num_items,
        ethashprime_hash1024* dataset) noexcept
      : ethashprime_epoch_context{epoch, light_num_items, light, l1, dataset_num_items},
